...
(async function poll() {
  console.log(`Waiting for database connection (timeout: ${timeoutMs}ms, poll interval: ${pollIntervalMs}ms)...`);
  ...
  while (Date.now() < until) {
    ...
    try {
      ...
      console.log(`Attempt ${attempts}: Waiting for database... (${error instanceof Error ? error.message : 'Unknown error'})`);
